...
struct TexelTuner {
    ...
    void train(string path) {
        ...
        // 			auto
        // newGrad=fitOneParam(database[j].second,param,evaluator.evaluatePositionDeterministic(mainBoard));
            ...
            for (int i = 0; i < database.size(); i += batchSize) {
                ...
                for (int j = i; j < min(int(database.size()), i + batchSize); j++) {
                    ...
                    if (numberOfPositions % 100'00 == 0)
                        cout << numberOfPositions << "/" << batchSize << ' '
                             << float(numberOfPositions) / batchSize * 100 << "%\n";
                }
                for (int p = 0; p < n; p++) {
                    grad[p] /= batchSize;
