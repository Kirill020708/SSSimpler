...
struct MoveGeneration {
    ...
    Bitboard computePinnedPiecesW(Board &board) {
        ...
        board.whitePinned = pinnedPieces;
    ...
    Bitboard computePinnedPiecesB(Board &board) {
        ...
        board.blackPinned = pinnedPieces;
    ...
    inline bool seeEval(Board &board, int square, int color, int firstAttacker, int threshold) {
        ...
        Bitboard allowed = (~(board.whitePinned | board.blackPinned)) | (board.whitePinned & whiteKingRay) | (board.blackPinned & blackKingRay);
        allowed |= board.kings;
        ...
        while (true) {
            ...
            if (attackingPiece == PAWN)
                isPromotion = (square < 8) || (square >= 56);
            ...
            int moveValue = pieceMaterial[capturedPiece] + isPromotion * (pieceMaterial[QUEEN] - pieceMaterial[PAWN]);

            score += moveValue;
            int pieceValue = pieceMaterial[attackingPiece];
            if (isPromotion)
                pieceValue = pieceMaterial[QUEEN];
